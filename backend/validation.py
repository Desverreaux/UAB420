...
def validateMoistureLevel(moistureLevel): 
     try:
        ...
        raise HTTPException(status_code=400, detail=f"Bad request: '{value}' is not a valid number")
...
def validateMoistureData(dataObject):
    ...
    if dataObject == None:
        raise HTTPException(status_code=400, detail=f"Bad request: No data was sent")
...
def validatePlantID(plantIdentifier):
    try:
        if(type(plantIdentifier) == int):
            return getPlantID_int(plantIdentifier)
            ...
            try:
                ...
                return getPlantID_int(plantId) #if the above line executes without throwing an error then the identifier was a number passed as a string
            except ValueError:
                return getPlantID_string(plantIdentifier)
    except TypeError: #Catches exceptions throw as a result of no input
        raise HTTPException(status_code=400, detail=f"Bad request: A value was not provided for plantID")
